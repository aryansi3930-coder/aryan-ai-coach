...
if audio_file:
    with st.spinner("Analyzing audio content and grammar logic..."):
        try:
            ...
            # 🎯 POWERFUL PROMPT ENGINE: Forces AI to return exact structural parsable variables
            prompt = [
            ...
            # Parsing variables safely from text blocks
    ...
    # 🔓 UI DISPLAYER PANELS
    st.write("---")
    st.markdown(f"### 🎙️ Session Feedback Analysis")
    ...
    # Show correction drawer card if mistake occurred
    if mistake_val == 1 or (corrected.lower() != "perfect" and corrected != ""):
        st.markdown(f"""
        <div class="feedback-card">
            <h4 style="color: #f43f5e; margin:0 0 5px 0;">⚠️ Grammar Correction Required:</h4>
            <p style="color: #e2e8f0; margin:0 0 5px 0;"><b>Sahi Answer:</b> {corrected}</p>
            <p style="color: #94a3b8; margin:0;"><b>Reason (Kyun?):</b> {reason}</p>
        </div>
        """, unsafe_allow_html=True)
    else:
    ...
    # 💾 PERMANENT DATABASE STORAGE LOGGING ENGINE
    conn = sqlite3.connect("aryan_robot_final_solid.db")
    cursor = conn.cursor()
    cursor.execute("""
